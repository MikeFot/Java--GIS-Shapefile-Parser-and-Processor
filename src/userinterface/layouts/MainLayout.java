...
public class MainLayout extends Thread implements DisposeListener, SelectionListener {
	...
	private Collection<ErgoPolyline> GEOMETRY_COLLECTION;
	...
	private Shell addMainShell(final Display firstDisplay) {
		...
		CreateWidgets.createMenuItems(mainShell); // Create Menu Items, should expand
	...
	private Shell addSearchShell(Shell shell) {
		...
		buttonSearch_1 = CreateWidgets.createButton(searchShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Clear", 1); // create draw button

		// Row 2
		listSearch = CreateWidgets.createList(searchShell, new GridData(GridData.FILL, GridData.FILL, true, true), 400, 400, 2); // create list populated with IDs

		// Row 3
		buttonSearch_2 = CreateWidgets.createButton(searchShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Set Source", 1); // create draw button
		label_13 = CreateWidgets.createLabel(searchShell, new GridData(GridData.FILL), comboSource.ergoCombo.getText(), 500, 1); // create info label

		// Row 4
		buttonSearch_3 = CreateWidgets.createButton(searchShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Set Target", 1); // create draw button
		label_14 = CreateWidgets.createLabel(searchShell, new GridData(GridData.FILL), comboTarget.ergoCombo.getText(), 400, 1); // create info label
	...
	private Shell addInfoShell(Shell shell) {
		...
		String crsDescription = ParseShapefile.getVerboseCRS();
	...
	private void addWidgets(Shell shell) {
		...
		// Row 1
		CreateWidgets.createLabel(myShell,new GridData(GridData.BEGINNING), "Browse for a ShapeFile: ", 200, 1); // create info label
		buttonBrowse = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Browse", 2); // create draw button

		// Row 2
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Using ShapeFile: ", 200, 1); // create info label
		label_3 = CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "No File Selected. Please Select a Shapefile.", 600, 2); // create info label

		// Row 3
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Shape Identifier", 200, 1); // create info label for the List beneath it
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Attribute List", 600, 1); // create info label for the List beneath it
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Coordinate List", 200, 1); // create info label for the List beneath it

		// Row 4
		listID = CreateWidgets.createList(myShell, new GridData(GridData.FILL, GridData.FILL, true, true), 200, 400, 1); // create list populated with IDs
		listAttributes = CreateWidgets.createList(myShell, new GridData(GridData.FILL, GridData.FILL, true, true), 500, 400, 1); // create list populated with metadata
		listCoordinates = CreateWidgets.createList(myShell, new GridData(GridData.FILL, GridData.FILL, true, true), 200, 400, 1); // create list populated with coordinates

		// Row 5
		label_8 = CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "CRS : Not Defined", 200, 1); // create info label
		buttonViewCRS = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Details", 2); // create button

		// Row 6
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Press to display map: ", 200, 1); // create info label
		buttonMap = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Open Map", 2); // create draw button

		// Row 7
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "EPSG File :", 200, 1); // create info label
		comboEPSG = CreateWidgets.createCombo(myShell, new GridData(GridData.BEGINNING), 400, 1); 
		new Label(myShell, SWT.NONE); // empty label, used to arrange widgets

		// Row 8
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Source Coordinate System :", 200, 1); // create info label
		comboSource = CreateWidgets.createCombo(myShell, new GridData(GridData.FILL), 400, 1);
		buttonDetectCRS = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Auto-Detect CRS", 1); // create draw button

		// Row 9
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Target Coordinate System :", 200, 1); // create info label
		comboTarget = CreateWidgets.createCombo(myShell, new GridData(GridData.FILL), 400, 1);
		buttonSearchCRS = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Search and Set CRS", 1); // create draw button

		// Row 10
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Export to CSV :", 200, 1); // create info label
		buttonRawCSV = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Export to CSV", 1); // create draw button
		new Label(myShell, SWT.NONE); // empty label, used to arrange widgets

		// Row 11
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Transform and Export to CSV :", 200, 1); // create info label
		buttonCSV = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Transform to CSV", 1); // create draw button
		new Label(myShell, SWT.NONE); // empty label, used to arrange widgets

		// Row 12
		CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Export to KML (to WGS 1984) :", 200, 1); // create info label
		buttonKML = CreateWidgets.createButton(myShell, new GridData(GridData.BEGINNING, GridData.CENTER, false, false), "Export to KML", 1); // create draw button
		new Label(myShell, SWT.NONE); // empty label, used to arrange widgets

		// Row 13
		label_13 = CreateWidgets.createLabel(myShell, new GridData(GridData.FILL), "Status : ", 200, 1); // create status label
		STATUS_LIST = CreateWidgets.createList(myShell, new GridData(GridData.FILL, GridData.FILL_HORIZONTAL, false, false), 100, 50, 1); // create list populated with IDs 
		...
		if (!fileName.isEmpty() || !fileName.equals("") || fileName != null) {
			...
			String[] listCRS = TableOperations.createComboString(fileName);
	...
	private String getShapefileSystem() {
		ErgoPolyline currentPolyline = Iterables.get(GEOMETRY_COLLECTION, 0);
	...
	private void setComboIndex(ErgoCombo inputCombo) {
		int EPSG_CODE = ParseShapefile.getEPSG_CODE();
	...
	public void widgetSelected(SelectionEvent selectionEvent) {
		...
		if (callerHashCode == buttonBrowse.getErgoID()) {
			...
			URL shpLocation = new FileOperations().browseAndReturnURL(); // user input for file
			...
			// Parse the ShapeFile
			try {
				GEOMETRY_COLLECTION = ParseShapefile.parseURLshapefile(shpLocation);
				if (GEOMETRY_COLLECTION == null) {
					changeStateOfWidgets(false);
					return;
				}
				...
				String[] plineIDList = new String[GEOMETRY_COLLECTION.size()];
				plineIDList = null;
				plineIDList = new String[GEOMETRY_COLLECTION.size()];
				int iter_PolylineCount = 0;

				for (ErgoPolyline singlePolyline : GEOMETRY_COLLECTION) {
			...
			} catch (NoSuchAuthorityCodeException eAuthority) {
				Log.Exception(eAuthority, 0);
				eAuthority.printStackTrace();
			} catch (IOException eIO1) {
				Log.Exception(eIO1, 0);
				eIO1.printStackTrace();
			}

		} else if (callerHashCode == buttonMap.getErgoID()) {
			Log.Out("Detected click on Info Button " + selectionEvent.getSource().hashCode(), 1, false);

			try {
				MapDisplay.shapefileDisplay(filePath.getPathAsFile());
				Log.Out("Displaying contents of file." , 1, true);

			} catch (IOException eIO2) {
				Log.Err("***Exception while displaying shapefile.***", 0 , true);
				Log.Exception(eIO2, 0);
				eIO2.printStackTrace();
			}

		} else if (callerHashCode == buttonKML.getErgoID()) {
			Log.Out("Detected click on myKML_Button " + selectionEvent.getSource().hashCode(), 1 , false);

			if (GEOMETRY_COLLECTION != null && REFERENCE_SETS.getSourceSystem() != null) {
				ExportKML.createKML(GEOMETRY_COLLECTION, REFERENCE_SETS.getSourceSystem());
			} else {
			...
			ErgoReferenceSystem selectedCRS = TableOperations.splitComboString(selectedItem);
			...
			if (!outString.equalsIgnoreCase("") && !outString.equalsIgnoreCase(null)) { // very important, keep it here or else everything breaks
				...
				if (selectionIndex <= GEOMETRY_COLLECTION.size() && selectionIndex >= 0) { // sanity check for selection

					ErgoPolyline currentPolyline = Iterables.get(GEOMETRY_COLLECTION, Integer.valueOf(outString.trim())-1);

					Log.Out("HashMapSize : " + currentPolyline.getHashMapSize(), 1, false);

					String[] stringKeysPline = currentPolyline.getStringKeys();

					String[] coordinateTable = TableOperations.buildCoordList(currentPolyline); // call method to create list items
					String[] attributeTable = TableOperations.buildAttributeList(stringKeysPline, currentPolyline);
